...
const createAccessToken = async (userId: string, sessionId: string) => {
...
const createRefreshToken = async (userId: string, sessionId: string) => {
	return await new SignJWT({ userId, sessionId })
		.setProtectedHeader({ alg: 'HS256' })
		.setIssuedAt()
...
const createTokensAndSession = async (userId: string, userAgent: string) => {
...
export const authService = {
	...
	login: async (email: string, password: string, userAgent: string) => {
	...
	refresh: async (oldRefreshToken: string, userAgent: string) => {
