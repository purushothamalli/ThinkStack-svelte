...
{#if data.evaluating}
	<div
		class="min-h-screen bg-stone-950 text-stone-200 flex flex-col items-center justify-center p-6 selection:bg-stone-500/30"
	>
		<div
			class="max-w-md w-full bg-stone-900/30 border-3 border-stone-850 rounded-3xl p-8 relative overflow-hidden text-center space-y-6"
		>
			<!-- Glow effects -->
			<div
				class="absolute -top-24 -left-24 w-48 h-48 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"
			></div>
			<div
				class="absolute -bottom-24 -right-24 w-48 h-48 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"
			></div>
			...
			<div class="relative w-24 h-24 mx-auto flex items-center justify-center">
				...
				<div
					class="absolute inset-0 rounded-full border-4 border-t-indigo-500 border-r-emerald-500 animate-spin"
				></div>
				<svg
			...
			<div class="space-y-2">
				...
				<p
					class="text-xs font-black tracking-widest uppercase text-indigo-400 min-h-6 transition-all duration-300"
				>
					{evalSteps[evalStep]}
				</p>
			</div>
			...
			<div class="bg-stone-950/50 border border-stone-850/50 rounded-2xl p-4 text-left">
				<p class="text-xs text-stone-500 leading-relaxed">
					We are processing your submission asynchronously using a background worker queue. This
					keeps the server responsive and prevents HTTP timeouts. The page will automatically
					refresh as soon as evaluation is completed.
				</p>
			</div>
		</div>
	</div>
{:else}
	<div class="min-h-screen bg-stone-950 text-stone-200 p-6 md:p-12 selection:bg-stone-500/30">
		<div class="max-w-7xl mx-auto space-y-8">
			...
			{#if isLoading}
				<div
					class="max-w-4xl mx-auto text-center py-20 space-y-6 flex flex-col items-center justify-center"
				>
					<div class="animate-spin rounded-full h-12 w-12 border-b-2 border-white"></div>
					<p class="text-stone-500 font-bold text-sm tracking-tight">
						Retrieving your evaluation results...
					</p>
				</div>
			{:else if !submission}
